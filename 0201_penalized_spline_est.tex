...
\begin{itemize}
    ...
    \item the sequence of knot vectors $(t_{(n)})_{n\in\N}$, where for each $n$ the vector $t_{(n)}\in \bigcup_{i=1}^\infty [a,b]^i$ is a partition of $[a,b]$ with $a = (t_{(n)})_0 < \cdots < (t_{(n)})_{k_n} = b$, bounded mesh ratio, and $k_n\rightarrow\infty$ as $n\rightarrow\infty$;
