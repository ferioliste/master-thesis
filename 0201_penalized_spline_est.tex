...
We define the expected utility $\Lambda:W^p\rightarrow\R$ as
\begin{equation*}
...
\end{equation*}
then, if $k_n\rightarrow\infty$ as $n\rightarrow\infty$, $\delta_n$ is asymptotically equivalent to $1/N_n$, that is $\delta_n \asymp 1/N_n$.
\end{proposition}
\begin{proof}\mbox{}
...
\end{equation*}
so $\delta_n\asymp 1/N_n$. In particular, $\delta_n\rightarrow0$ as $n\rightarrow\infty$.
\end{proof}
...
\begin{itemize}
    ...
    \item the sequence of numbers of inner knots $(k_n)_{n\in\N}$ and the sequence knot vectors $(t_{(n)})_{n\in\N}$, where for each $n$ the vector $t_{(n)}\in[a,b]^{k_n}$ is a partition of $[a,b]$ with $a = (t_{(n)})_0 < \cdots < (t_{(n)})_{k_n+1} = b$, bounded mesh ratio, and $k_n\rightarrow\infty$ as $n\rightarrow\infty$;
    \item the sequence of penalty parameters $(\lambda_n)_{n\in\N}\subset (0,\infty)$, such that $\lambda_n\rightarrow 0$ as $n\rightarrow\infty$;
    \item the penalty order $q$.
\end{itemize}
Note that $m$ together with $k_n$ and $(t_{(n)})_{n\in\N}$ allows to define the sequence of spline spaces $(\mathbb G_n)_{n\in\N}$. For the ease of notation, we will keep writing $\mathbb G$ instead of $\mathbb G_n$, $t$ instead of $t_{(n)}$, and $k$ instead of $k_n$, keeping in mind that they all still depend on $n$.

We further impose the following constraints:
\begin{itemize}
    \item $q\leq m$, otherwise the $q$-th derivative in $J_q$ cannot be calculated;
    \item $p>1/2$, as we will later require in the proof of \autoref{thm:approx_error};
    \item $q>1/2$, as we will later require in \autoref{prop:gu}.
\end{itemize}
