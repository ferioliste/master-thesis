...
Note that $\mathbb G$ is univocally identified by the knots vector $t\in\R^{k+2}$ and the degree $m$. In general, $t$ and so $\mathbb G$ and $N$ can change with $n$, but $m$ must remain constant. When we write $\mathbb G_n$ and $N_n$ we explicit the dependence on $n$.
...
\begin{itemize}
    ...
    \item the sequence of knot vectors $(t_{(n)})_{n\in\N}$, where for each $n$ the vector $t_{(n)}\in \bigcup_{i=1}^\infty [a,b]^i$ is a partition of $[a,b]$ with $a = (t_{(n)})_0 < \cdots < (t_{(n)})_{k_n} = b$ and bounded mesh ratio;
