...
The aim of this section is to prove Theorems \ref{thm:approx_error} and \ref{thm:est_error}. The proofs of the main points are given in \cite{huang2021}. Here, we describe in detail all the steps and fill in the parts that are only sketched or omitted.
...
We are now ready to prove \autoref{thm:approx_error}. The proof of conclusion (c) is taken and integrated from \cite{huang2021}. The proofs of conclusions (a) and (b) are omitted in \cite{huang2021}, but we derive them below.
...
Using again \autoref{lem:convexity}, we prove \autoref{thm:est_error}. The proof of conclusion (b) is taken and integrated from \cite{huang2021}. The proof of conclusion (a) is omitted in \cite{huang2021}, but we derive it below.
...
\begin{equation*}
    ...
    \leq A_n L(\hat\eta_n) = A_n a \bigg( \frac{1}{n \lambda_n^{1/(2q)}} \wedge \frac{1}{n \delta_n} \bigg)^{1/2} 
    = o(1).
\end{equation*}
Then
\begin{equation*}
