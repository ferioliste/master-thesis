\subsection{Proofs of the master theorems}\label{sec:proofs}
We start by stating and proving \autoref{lem:convexity}, that is used in the proofs of both theorems.
...
\omega_{m-p}\!\left(f^{(p)}; \delta_n\right)_2.
\end{equation*}
...
Let $(X,d)$ be a metric space and let $K \subset X$ be a compact set. 
...
We are now ready to prove \autoref{thm:approx_error}.
...
Let $g$ such that $L(g)=s$, where both $g$ and $s$ depend on $a$ and $n$. By definition of $A_n$, we have
\begin{equation*}
...
In addition,
\begin{align*}
    a\left(\delta_n^p + \lambda_n^{1/2}\delta_n^{(p-q)\wedge 0}\right) 
...
First, assuming we can exchange differentiation and expectation, we have that
\begin{equation*}
    \E\big[\dot l[\bar\eta_n, g]\big] = \frac{\de}{\de\alpha}\Lambda(\bar\eta_n+\alpha g)\Bigg|_{\alpha=0^+}.
\end{equation*}
In addition,
\begin{gather*}
    \frac{\de}{\de\alpha}J_q(g) = \frac{\de}{\de\alpha}\int_{\mathcal U} \{(\bar\eta_n+\alpha g)(x)\}^2 \de x \\
    = \int_{\mathcal U} \frac{\de}{\de\alpha}\big(\{(\bar\eta_n+\alpha g)(x)\}^2\big) \de x = \int_{\mathcal U} 2\,g(x)\big((\bar\eta_n+\alpha g)(x)\big) \de x,
\end{gather*}
and
\begin{equation*}
    \frac{\de}{\de\alpha}J_q(g)\Bigg|_{\alpha=0^+} = \int_{\mathcal U} 2\,g(x)\big((\bar\eta_n+\alpha g)(x)\big) \de x\Bigg|_{\alpha=0^+} = 2\int_{\mathcal U} \bar\eta_n(x)g(x) \de x = 2J_q(\bar\eta_n, g)
\end{equation*}

Since $\bar\eta_n$ is the maximizer of the concave functional $\p\Lambda(\cdot)$ over $\mathbb G$, it satisfied the condition
\begin{equation*}
...
and the continuous functional
\begin{equation*}
    L(g) = \|g-\bar\eta_n\| + \lambda_n^{1/2} J_q^{1/2} (g - \bar\eta_n)
\end{equation*}
on $\mathcal C = \mathbb G$. Note that $L(\cdot)$ is continuous since
\begin{gather*}
    |L(g_1) - L(g_2)| = |\|g_1-\bar\eta_n\| + \lambda_n^{1/2} J_q^{1/2} (g_1 - \bar\eta_n) - \|g_2-\bar\eta_n\| - \lambda_n^{1/2} J_q^{1/2} (g_2 - \bar\eta_n)| \\
    \leq |\|g_1-\bar\eta_n\| - \|g_2-\bar\eta_n\| | + | \lambda_n^{1/2} J_q^{1/2} (g_1 - \bar\eta_n) - \lambda_n^{1/2} J_q^{1/2} (g_2 - \bar\eta_n)| \\
    \leq \|g_1 - g_2\| + \lambda_n^{1/2} J_q^{1/2} (g_1 - g_2)|
\end{gather*}
by reverse triangle inequality.

We take $\displaystyle s^2 = a^2\Big(\frac{1}{n \lambda_n^{1/(2q)}} \wedge \frac{1}{n \delta_n}\Big)$, with $a>0$.

We consider now the quantity $\displaystyle \frac{\partial}{\partial\alpha}C(\bar\eta_n + \alpha(g-\bar\eta_n))\Bigg|_{\alpha=1^+}$.
...
\end{equation*}
and, using \autoref{eq:E_difference},
\begin{gather*}
...
Let $g \in \mathbb G$ with $L(g) \leq s$. By Condition 1,
\begin{equation*}
\begin{aligned}
    |\mathrm{I}| 
&= \big(\|g - \bar\eta_n\|^2 
...
&= L(g)^{1/2}\,O_P\!\Big(\frac{s}{a}\Big)\leq s\,O_P\!\Big(\frac{s}{a}\Big) 
= O_P\!\left(\frac{s^2}{a}\right).
\end{aligned}
\end{equation*}

By Condition 5,
\begin{equation*}
    \|g - \tilde{\eta}_n\|_\infty 
    \leq A_n \|g - \tilde{\eta}_n\| 
    = A_n a \left( \frac{1}{n \lambda_n^{1/(2q)}} \wedge \frac{1}{n \delta_n} \right)^{1/2} 
    = o(1).
\end{equation*}
Then, for $n$ large enough, $\|g - \tilde{\eta}_n\|_\infty \leq B$. Then, by Condition 2, for $g \in \mathbb G$ with $L(g) = s$,
\begin{align*}
\mathrm{II} &\geq M \|g - \bar\eta_n\|^2 + 2\lambda_n J_q(g - \bar\eta_n) \\
&\geq \frac{1}{2}(M \wedge 2) \big(\|g - \bar\eta_n\| 
   + \lambda_n^{1/2} J_q^{1/2}(g - \tilde{\eta}_n)\bigr)^2 \\
...
\begin{equation*}
\mathrm{I} + \mathrm{II} \geq \frac{1}{2}(M \wedge 2)s^2 - O_P\!\Big(\frac{s^2}{a}\Big) > 0.
\end{equation*}
and, thus,
\begin{equation*}
    \frac{\partial}{\partial\alpha}C(\bar\eta_n + \alpha(g-\bar\eta_n))\Bigg|_{\alpha=1^+} > 0\qquad\text{for }g\in\mathbb G\text{ with }L(g)=s.
\end{equation*}

Then \autoref{lem:convexity} implies that the minimizer $\hat\eta_n$ of $C(\cdot)$ in $\mathbb G$ satisfies $L(\hat\eta_n)\leq s$. Then
\begin{equation*}
    \|\hat\eta_n - \bar\eta_n\|^2 
    + \lambda_n J_q(\hat\eta_n - \bar\eta_n) 
    \leq s^2 
    = a^2 \Big( \frac{1}{n \lambda_n^{1/(2q)}} \wedge \frac{1}{n \delta_n} \Big).
\end{equation*}
