\subsection{Checking the probabilistic bounds' hypotheses}

Theorems \ref{thm:approx_error} and \ref{thm:est_error} have quite technical hypotheses. In this section, we give one proposition and three lemmas that provide simple sufficient conditions to check these assumptions in practice. The lemmas and their proofs are originally taken and integrated from \cite{huang2021}. While \autoref{prop:thm_512} is also presented in \cite{huang2021}, we show how it can be used to derive sufficient conditions for condition (3) of \autoref{thm:approx_error} and condition (4) of \autoref{thm:est_error}. Finally, we present and prove a variation of \autoref{lem:cond_2}, \autoref{lem:cond_2_bis}, that is strictly better in most cases.

We start considering condition (3) of \autoref{thm:approx_error} and condition (4) of \autoref{thm:est_error}. These are conditions on the sequence of spline spaces $(\mathbb G_n)_{n\in\N}$ and the sequence of penalty parameters $(\lambda_n)_{n\in\N}$. We use the following result from \cite{constructive_approximation}.

\begin{proposition}[Theorem 5.1.2 of \cite{constructive_approximation}]\label{prop:thm_512}
    Take $A_n$ as in \autoref{def:An}. Under bounded mesh ratio condition, we have that $A_n \lesssim \delta_n^{-1/2}$. In other words, there exists a constants $C>0$ such that $A_n\leq C\delta_n^{-1/2}$ for $n$ large enough.
\end{proposition}

Since
\begin{equation*}
    0 \leq A_n^2 \big(\delta_n^{2p'} \vee (\lambda_n \delta_n^{2(p'-q)\wedge 0})\big) \leq C^2\delta_n^{-1}\big(\delta_n^{2p'} \vee (\lambda_n \delta_n^{2(p'-q)\wedge 0})\big)
\end{equation*}
and
\begin{equation*}
    0 \leq A_n^2 \bigg(\frac{1}{n \lambda_n^{1/(2q)}} \wedge \frac{1}{n \delta_n}\bigg) \leq C^2\delta_n^{-1}\bigg(\frac{1}{n \lambda_n^{1/(2q)}} \wedge \frac{1}{n \delta_n}\bigg),
\end{equation*}
using the squeeze theorem, we find that sufficient conditions for condition (3) of \autoref{thm:approx_error} and condition (4) of \autoref{thm:est_error} are
\begin{equation*}
    \lim_{n\rightarrow\infty} \delta_n^{-1}\big(\delta_n^{2p'} \vee (\lambda_n \delta_n^{2(p'-q)\wedge 0})\big) = 0
\end{equation*}
and
\begin{equation*}
    \lim_{n\rightarrow\infty} \delta_n^{-1}\bigg(\frac{1}{n \lambda_n^{1/(2q)}} \wedge \frac{1}{n \delta_n}\bigg) = 0.
\end{equation*}

Since these are conditions on the sequence of spline spaces $(\mathbb G_n)_{n\in\N}$ and the sequence of penalty parameters $(\lambda_n)_{n\in\N}$, they can be verified once and for all. In particular, after fixing the sequences the same conditions apply regardless of the utility function used to define the estimator. For this reason, in \autoref{sec:chapter2}, we will not check these conditions.

We now provide three lemmas for verifying condition (1) of \autoref{thm:approx_error} and conditions (1) and (2) of \autoref{thm:est_error}. These lemmas correspond, in order, to lemmas 5.1, 5.3, and 5.2 of \cite{huang2021}.
...
\begin{enumerate}
    ...
    \item[(ii)] For $g \in\mathbb G$, $\ell(\bar\eta_n + \alpha g)$ as a function of $\alpha$ is twice continuously differentiable and there are constants $B > 0$ and $M > 0$ such that
    \begin{equation*}
        \frac{\de^2}{\de\alpha^2} \ell(\bar\eta_n + \alpha g) \leq -M \|g\|_2^2, \qquad\text{ for }0\leq\alpha\leq1,
    \end{equation*}
    holds for $g \in\mathbb G$ with $\|g\|_\infty \leq B$, with probability tending to $1$ as $n \rightarrow \infty$.
\end{enumerate}
Then, condition (2) of \autoref{thm:est_error} is also satisfied, that is there are constants $B^* > 0$ and $M^* > 0$ such that, with probability tending to $1$ as $n \to \infty$, we have that
\begin{equation*}
...
About \autoref{lem:cond_3}, we notice that in practice we do not need to check for condition (i) as it is a result of \autoref{thm:approx_error}.
...
\begin{proposition}[Result from Section 3 of \cite{utreras}]\label{prop:utreras}
...
By Theorem 3.3.1 of \cite{weinberger}, since $V(h,h)$ is completely continuous with respect to $J_q(h,h)$, $V$ and $J_q$ can be simultaneously diagonalized. This means that there exists a sequence of eigenfunctions $\phi_i\in L^2$, for $i=1,2,\dots$, and a sequence of eigenvalues $\rho_i$,  for $i=1,2,\dots$, such that
\begin{equation*}
