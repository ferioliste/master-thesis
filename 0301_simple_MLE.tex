...
\end{equation*}
then $u^\tr\tilde\varphi(\mathbf Y)=c$ almost surely for some constant $c$. Since $\mathrm{supp}(\mathbf Y) = [a,b]$, using \autoref{lem:support}, we have $u^\tr\tilde\varphi(x)=c$ for all $x\in[a,b]$. Since $0$ is the only constant in $\mathbb G^\times$, we have $u^\tr\tilde\varphi(x)=0$ for all $x\in[a,b]$ and so $u=0$.
\end{proof}
