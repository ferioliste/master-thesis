\subsection{Shifted Maximum Likelihood}
In this section, we present a modified version of the classical maximum likelihood estimator of the density. This will allow us to remove the assumption that $f_0$ is bounded away from $0$, at the cost of having a (slightly) unbiased estimator.
...
\end{equation*}
Equivalently, $\widetilde{\mathbf X}$ is distributed such that
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
where $\mathbf X$ has density $f_0$ on $[a,b]$ and $\mathbf U$ is independent of $\mathbf X$ and uniformly distributed on $[a,b]$. It is immediate to check that $\tilde f_0$ satisfies the properties of a valid probability density on $[a,b]$. We will construct an estimator of $\tilde f_0$.

As in \autoref{sec:simple_MLE}, we represent $f \in W_p\cap\mathcal D$ as
\begin{equation*}
...
\begin{gather}
    ...
    = (1-\gamma)\E\big[\eta(\mathbf X)\big] + \gamma\int_a^b\eta(x)\de x - \ln \int_a^b \exp \eta(x)\de x, \label{eq:Lambda_shifted_MLE}
\end{gather}
so that $\ell$ can be defined as
\begin{gather}
    \ell(\eta; \mathbf X_1 , \ldots, \mathbf X_n) = \frac{1}{n}\sum_{i=1}^n l(\eta; \mathbf X_i) \notag \\
    = \sum_{i=1}^n \bigg((1-\gamma)\eta(\mathbf X_i) + \gamma\int_a^b\eta(x)\de x - \ln \int_a^b \exp \eta(x)\de x\bigg). \label{eq:ell_shifted_MLE}
\end{gather}
...
We will show that $\Lambda$, $\ell$ and $l$ satisfy the hypotheses of Lemmas $\ref{lem:cond_1}$, \ref{lem:cond_3}, and \ref{lem:cond_2}, so that we are able to apply Theorems \ref{thm:approx_error} and \ref{thm:est_error}.
...
In our case,
\begin{equation*}
    l(\eta, \mathbf X) = (1-\gamma)\eta(\mathbf X) + \gamma\int_a^b\eta(x)\de x - \ln \int_a^b \exp \eta(x)\de x.
\end{equation*}
Then,
\begin{gather*}
    l(\bar\eta_n+\alpha g) = (1-\gamma)\big(\bar\eta_n(\mathbf X) +\alpha g(\mathbf X)\big) \\
    + \gamma\int_a^b(\bar\eta_n(x)+\alpha g(x))\de x - \ln\int_a^b\exp(\bar\eta_n(x)+\alpha g(x))\de x, \\
    \frac{\de}{\de\alpha}l(\bar\eta_n+\alpha g) = (1-\gamma)g(\mathbf X) + \gamma\int_a^b g(x)\de x - \frac{\int_a^bg(x)\exp(\bar\eta_n(x)+\alpha g(x))\de x}{\int_a^b\exp(\bar\eta_n(x)+\alpha g(x))\de x}, \text{ and}\\
    \dot l[\bar\eta_n; g]=(1-\gamma)g(\mathbf X) + \gamma\int_a^b g(x)\de x - \frac{\int_a^bg(x)\exp(\bar\eta_n(x))\de x}{\int_a^b\exp(\bar\eta_n(x))\de x}.
\end{gather*}
It follows that
\begin{gather*}
...
\end{equation*}
Unfortunately, this completely ruins the regularity of $f_2$ since it introduces corner points where the first derivative is discontinuous. Instead, we define
\begin{equation*}
    f_4(x) = f_2(x) + 0 \wedge\min_{y\in[a,b]} f_2(y) \Big/ \Big(1+(b-a)\big(0 \wedge\min_{y\in[a,b]} f_2(y)\big)\Big),
\end{equation*}
where the minimum of $f_2$ over $[a,b]$ always exists since $f_2$ is continuous. Intuitively, the estimator $f_4$ is $f_2$ shifted up by the bare minimum to make $f_2$ positive for all $x\in[a,b]$. In addition, since by conclusion a of \autoref{thm:overall_error}, we have that $f_4$ is an asymptotically unbiased estimator.