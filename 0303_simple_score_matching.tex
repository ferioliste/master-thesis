...
\end{equation*}
If $\E\big[\big(u^\tr\varphi(\mathbf X)\big)^2\big]=0$, then $u^\tr\varphi(\mathbf X) = 0$ almost surely. Since $\mathrm{supp}(\mathbf X) = [a,b]$ by hypothesis, using \autoref{lem:support}, we have $u^\tr\tilde\varphi(x)=0$ for all $x\in[a,b]$ and so $u=0$.
\end{proof}
