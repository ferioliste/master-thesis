...
\end{equation*}
If $\E\big[\kappa\big(\phi(\mathbf X)\big)\big(u^\tr\varphi(\mathbf X)\big)^2\big]=0$, then $u^\tr\varphi(\mathbf X) = 0$ almost surely, since $\kappa\big(\phi(x)\big)$ is strictly positive for $x\in(a,b)$. Finally, we can use the same argument as in the proof of \autoref{prop:simple_sm_valid} to prove that $u=0$.
\end{proof}
