...
In this section, we show how the Simple Maximum Likelihood estimator can be implemented. We recall that
\begin{equation*}
...
\begin{equation*}
    \theta_0 = 0,
\end{equation*}
that also guarantees a one-to-one correspondence between $\eta=\theta^\tr \varphi(x)$ and the estimated density $f$.
...
The $\alpha$-point Gauss-Legendre quadrature rule is exact for all polynomials of degree at most $2\alpha-1$ and is very precise with all sufficiently smooth functions. We can approximate integrals on any interval using a simple change of variable.
...
Let $\theta\in\R^{m+k+1}$. Following the same reasoning as in the proof of \autoref{prop:MLE_valid}, we consider a random variable $\mathbf Y$ on $[a,b]$ with density
\begin{equation*}
    f(x) = \exp\!\big(\theta^\tr \varphi(x)\big) \Big/ \int_a^b\exp\!\big(\theta^\tr \varphi(x)\big)\de x,\qquad\text{for }x\in[a,b].
\end{equation*}
Then, the covariance matrix of $\varphi(\mathbf Y)$ is equal to
\begin{equation*}
