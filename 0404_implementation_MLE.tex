...
For $i\in\{0,\ldots,m\}$ and $j\in\{1,\ldots,k\}$,
\begin{gather*}
    I^{(2)}_{i,m+j} = \int_a^b x^i(x-t_j)_+^m\exp\!\big(\theta^\tr \varphi(x)\big)\de x = \int_{t_j}^b x^i(x-t_j)^m\exp\!\big(\theta^\tr \varphi(x)\big)\de x = \\
    \sum_{v=i}^k \int_{t_v}^{t_{v+1}} x^i(x-t_j)^m\exp\!\big(\theta^\tr \varphi(x)\big)\de x = \sum_{v=i}^k \int_{t_v}^{t_{v+1}} \sum_{z=0}^{m} \tbinom{m}{z} x^{i+m-z} (-t_j)^z \exp\!\big(\theta^\tr \varphi(x)\big)\de x = \\
    \sum_{v=i}^k \sum_{z=0}^{m} \tbinom{m}{z}(-t_j)^z \int_{t_v}^{t_{v+1}} x^{i+m-z} \exp\!\big(\theta^\tr \varphi(x)\big)\de x = \sum_{v=i}^k \sum_{z=0}^{m} \tbinom{m}{z}(-t_j)^z A_{v,(i+m-z)}.
\end{gather*}

With a bit more work one can cover all the other cases to calculate all the entries of $I^{(0)}$, $I^{(1)}$, and $I^{(2)}$.

\begin{algorithm}[H]
\caption{Newton-Raphson algorithm for the Simple Maximum Likelihood method}
