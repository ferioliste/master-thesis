...
Also in this case, we have that $\mathcal L$ verifies the hypotheses of \autoref{prop:newton_global_max}. First, we showed that $\mathcal L$ is twice differentiable at every $\theta\in\R^{m+k+1}$. In addition, for $\theta\in\R^{m+k+1}$, $\H(\theta)$ is the same as for the Simple Maximum Likelihood method and therefore it is negative semi-definite.