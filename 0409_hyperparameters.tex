...
While we have requirements on the asymptotic behavior of $\lambda_n$ as $n\rightarrow\infty$, for a fixed sample size $n$ the value of $\lambda_n$ must be chosen from the observed sample. In this section, we present two classical methods to select $\lambda_n$: simple validation and $K$-fold cross-validation. In both cases, we compare candidate values of $\lambda_n$ by evaluating how well the estimated density performs on unseen data. Since all the methods ultimately produce density estimates, we use the average log-likelihood on the validation set to compare the different values of $\lambda_n$.
