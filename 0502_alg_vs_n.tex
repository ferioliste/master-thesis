...
For each configuration and each sample size $n$, we first perform $5$ pilot runs in order to select the penalty parameter $\lambda_n$ by k-fold cross-validation, as done in the previous section. After fixing $\lambda_n$, we generate three additional independent samples and fit the estimator on each of them.
