...
For the pick-up times, we restrict the analysis to the daytime interval from $06{:}00$ to $22{:}00$ and we extract a random subsample of $n=1000$ observations. We make the working assumption that the observations are independent and identically distributed.

For the trip distance, we remove trips with distance equal to $0$ and we discard trips longer than $13$ km, since they represent only a small fraction of the data and would dominate the scale of the plot. We consider a random subsample of $n=1000$ observations. Again, we the working assumption that the observations are independent and identically distributed.
...
We estimate the pick-up time density using two methods: Simple Maximum Likelihood and Generalized Score Matching. Using $K$-fold cross-validation, we select $\lambda_n=10^{-7}$ for both estimators. As a baseline, we also report a Gaussian kernel density estimator (KDE). 
...
One noticeable difference between the Simple Maximum Likelihood estimate and the Generalized Score Matching estimate is that, in the first, the density increases again as $22{:}00$ approaches. As in the synthetic experiments, the Generalized Score Matching estimate shows a poorer behavior close to the boundary of the support. Both spline-based methods appear to perform better than the KDE, especially near the boundaries. They also have the practical advantage of being much faster to evaluate, since the cost of evaluating the KDE at a point grows linearly with $n$.
...
We estimate the trip distance density using two methods: Shifted Maximum Likelihood and Simple Score Matching. For the Shifted Maximum Likelihood, we make the estimator asymptotically unbiased using the procedure outlined at the end of \autoref{sec:shifted_MLE}. Using $K$-fold cross-validation, we select $\lambda_n=10^{-6}$ for both estimators. As before, we report a Gaussian kernel density estimator (KDE). 
